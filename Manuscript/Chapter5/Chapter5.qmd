...
A Savitzky-Golay smoothing function with a 5 nm moving window was applied to each spectrum using the “hsdar” package in R [@hsdar]. The second derivative at 665 nm ($\delta\delta_{665}$), showing the highest variability between the control and the treatment, was tested as an indicator of the spectral changes following HWs.
...
where $R(560)$, $R(740)$, and $R(842)$ represent the reflectance at 560, 740, and 842 nm, respectively, and $\tau$ is equal to 0.64. These wavelengths were selected to align with the spectral resolution of satellites missions such as Sentinel-2, for broader remote sensing application. 
...
This annex shows the results of gams modelling the change of Radiometric indices with duration of experiments.
...
# table_gams <- flextable_table %>% 
#   flextable::fontsize(size = 10, part = "header") %>%
#   flextable::fontsize(size = 8) %>% 
#   flextable::width(j = c("Index"), width = 0.2) # Set narrower columns
# 
# save_as_image(table_gams, path = "Figs/table_gams1.png")

knitr::include_graphics("Figs/table_gams1.png")
...
This annex shows the results of gams modelling the change of SHSI with exposure times
...
# tblSHSI <- table_gam_SHSI %>% 
#   flextable::fontsize(size = 10, part = "header") %>%
#   flextable::fontsize(size = 8) %>% 
#    flextable::width(j = c("Term"), width = 1) # Set narrower columns
# 
# save_as_image(tblSHSI, path = "Figs/table_gams2.png")

knitr::include_graphics("Figs/table_gams2.png")
